...
test('word book uses the daily-review winner for a duplicate word across sources', async ({ page }) => {
  ...
  await seed(page, {
    unlockedLevels: ['g2s1u1'], completedLevels: [], wordCards: [], stories: [],
    practiceByLevel: {
      g2s1u1: { 洗手: reviewPractice({ correctStreak: 1, wrongCount: 4 }) },
      'custom-duplicate-source': { 洗手: reviewPractice({ correctStreak: 3, wrongCount: 0 }) },
    },
    levelStars: {},
  ...
  await expect(filters.getByRole('button', { name: /待巩固.*1/ })).toBeVisible();
  await expect(filters.getByRole('button', { name: /已经熟悉.*0/ })).toBeVisible();
  await expect(page.locator('.wordbook-sections').getByText('洗手', { exact: true })).toHaveCount(1);
  await expect(page.getByText('重复词来源', { exact: true })).toHaveCount(0);
